...
export function TransactionRow({ postedAt, description, category, amount }) {
  return (
    <Link
    ...
    >
      ...
      <span className="col x2">{category}</span>
      <span className="col x2">{formatAmount(amount)}</span>
      <span className="col x15">{formatDate(postedAt)}</span>
    </Link>
  );
}

export function TransferRow({ postedAt, recipientEmail, memo, amount }) {
  return (
    <div className="table-row transfer-row">
      ...
      <span className="col x2">{recipientEmail}</span>
      <span className="col x2">{formatAmount(amount)}</span>
      <span className="col x15">{formatDate(postedAt)}</span>
    </div>
  );
